...
@if(count($BlockCategories) >0)
    <section id="landing-block-{{ @$TopicBlock->id }}" class="categories landing-block {{ @$TopicBlock->css_classes  }}" style="{{ $block_style }}">
        <div class="container">
            ...
            @endif
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4 meu-stagger meu-stagger-pop">
                @foreach($BlockCategories as $Category)
                    ...
                    <div class="col">
                        <a href="{{ $category_link_url }}">
                            <div class="meu-tile">
                                @if($Category->photo !="")
                                    <div class="meu-tile-icon {{$Category->photo !=""?"meu-tile-icon-image":""}}">
                                        <img loading="lazy"
                                             src="{{ route("fileView",["path" =>'sections/'.$Category->photo ]) }}?w=200&h=200&r=fit" alt="{{ $title }}"/>
                                    </div>
                                @elseif($Category->icon !="")
                                    <div class="meu-tile-icon">
                                        <i class="{{ $Category->icon }}"></i>
                                    </div>
                                @endif
                                <h5>{{ $title }}</h5>
                                <p>{!! nl2br($Category->$details) !!}</p>
                                <span class="meu-tile-arrow"><i class="fa-solid fa-arrow-{{ (@Helper::currentLanguage()->direction=="rtl")?"left":"right" }}"></i></span>
                            </div>
                        </a>
                    </div>
                @endforeach
