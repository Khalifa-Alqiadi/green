...
@push('after-styles')
    <link href="{{ URL::asset('assets/frontend/css/topic-page.css') }}?v={{ Helper::system_version() }}" rel="stylesheet"/>
@endpush

@section('content')
    <div class="meu-topic-view">
        <?php
        ...
        ?>
        <section class="breadcrumbs topic-hero d-flex align-items-center justify-content-center" style="background-image: url({{$category_image}})">
            <div class="container">
                <div class="topic-hero-content">
                    <span class="topic-hero-eyebrow" aria-hidden="true"></span>
                    <h1>{!! (@$WebmasterSection->id ==1)?$title:$page_title !!}</h1>
                    <ol>
        ...
        @endif
        <section id="content" class="topic-content-section">
            <div class="container topic-page">
                <div class="row g-4 g-xl-5 align-items-start topic-layout">
                    @if($Categories->count() >1)
                        @include('frontEnd.layouts.side')
                    @endif
                    <div
                        class="topic-main-column col-lg-{{($Categories->count()>1)? "9":"12"}} col-md-{{($Categories->count()>1)? "7":"12"}} col-sm-12 col-xs-12">
                        <article class="topic-article-card mb-4">
