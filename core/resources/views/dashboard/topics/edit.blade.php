            ...
            locale:'{{ @Helper::currentLanguage()->code }}'
                  ...
                  <span class="fa fa-calendar"></span>
                            ...
                            @if($WebmasterSection->authors_status)
                                <?php $selectedAuthors = $Topic->authors->pluck('id')->toArray(); ?>
                                <div class="form-group row">
                                    <label for="authors" class="col-sm-2 form-control-label">الكتاب / Authors</label>
                                    <div class="col-sm-10">
                                        <select name="authors[]" id="authors" class="form-control select2-multiple" multiple ui-jp="select2" ui-options="{theme: 'bootstrap'}">
                                            @foreach($AuthorsList as $Author)
                                                <option value="{{ $Author->id }}" {{ in_array($Author->id, $selectedAuthors) ? 'selected' : '' }}>{{ $Author->name_ar }}{{ $Author->name_en ? ' / '.$Author->name_en : '' }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            @else
                                <input type="hidden" name="authors[]" id="authors" value="{{ $Topic->author_id }}" ui-jp="select2" ui-options="{theme: 'bootstrap'}">
                            @endif
